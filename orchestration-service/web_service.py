"""These modules provides HTTP Gateway Service and JSON parsing"""
...
def application(request):
    ...
    if request.path == '/job':
        ...
        The '/job' GET request it can take a URL parameter of 'nextjob' with no value or a URL parameter of 'jobid' with an value.
        ...
        The body of the POST request contains JSON which is parsed into a dictionary named updated_values and passed to the method 'job.set_job(updated_values)'
        """
        ...
        For the GET request when there are no parameters call  'jobs.get_all()' and return statuses for all jobs.
        If content-type is text-html returns html
        if content-type is application/json returns json
        if content-type is text/plain return a string

        The POST request '/status' must have a jobid parameter and value.
        For the POST request parse the json in the body as a dictionary and pass the dictionary to 'jobs.set_job(data)'
        ...
        if request.method == 'GET':
            ...
            if content_type == 'text/html':
                ...
                return Response("<br>".join([f"{k}: {v}" for item in result for k, v in item.items()]), content_type='text/html')
            elif content_type == 'application/json':
                return Response(json.dumps(result), content_type='application/json')
            elif content_type == 'text/plain':
                return Response("\n".join([f"{k}: {v}" for item in result for k, v in item.items()]), content_type='text/plain')
